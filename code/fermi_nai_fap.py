...
def load_fermi_nai_data(fpath):
    """
    Load the Fermi/GBM NaI data from 
    ...
    Returns
    -------
    lc_nai : stingray.Lightcurve object
...
def main():
    ...
    # load Fermi/GBM NaI data
    fpath = datadir+"bn230307656_na_mete.sav"
    lc = load_fermi_nai_data(fpath)
    ...
    drw_logz_file = f"{datadir}fermi_nai_sim_drw_logz.txt"
    qpo_logz_file = f"{datadir}fermi_nai_sim_qpo_logz.txt"
   ...
   #     f.write("# logz \t d_logz \n")
    ...
    for i, idx in enumerate(idx_all):
        ...
        np.savetxt(f"{datadir}fermi_nai_lcsim{i}.dat", np.array([lcsim.time, lcsim.counts]).T)
        ...
        with open(f"{datadir}fermi_nai_drw_res_sim{i}.pkl", "wb") as f:
        ...
        with open(f"{datadir}fermi_nai_drw_qpo_res_sim{i}.pkl", "wb") as f:
